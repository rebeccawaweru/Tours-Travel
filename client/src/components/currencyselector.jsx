    ...
    {dropdown &&  <Box borderRadius={5} sx={{overflowY:"scroll"}} width="80%" style={styles.container}>
         ...
         <Grid maxWidth borderRadius={5} container>
          {(rates && rates.length > 0) ? rates.map((rate)=>{
            return <Typography component={Grid} item xs={12} md={3} marginBottom={4} color="black" onClick={()=> {handleChangeCurrency(rate.currency); setDropdown(false)}}>{rate.currency}</Typography>
