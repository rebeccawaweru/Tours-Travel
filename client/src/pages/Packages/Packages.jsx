    ...
    return (
       ...
       <Container maxWidth sx={{backgroundColor: 'whitesmoke',paddingY:"20px", marginTop:{xs:-4,md:-20}}}>
            ...
            {!search && combined && combined.length > 0 && combined.map((item)=>{
                    return <Package id={item._id} link={item.link} price={`${item.currency} ${Number(item.price).toLocaleString() || 0}`} location={`${item.hotel} ${item.location} ${item.country}`} title={item.title} duration={`${item.nights}nights ${item.days}days`} image={item.poster}/>
                  })
               }
