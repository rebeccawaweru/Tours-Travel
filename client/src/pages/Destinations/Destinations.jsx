...
export default function Destinations(){
    ...
    return (
       ...
       <Container maxWidth sx={{marginBottom:3, marginTop:{xs:12,sm:28,md:10}}}>
        ...
        <Typography variant="h4" fontWeight="bold">{t('destination.heading')}</Typography>
        </Stack>
        <Typography color="whitesmoke" variant="body1">{t('destination.caption')}</Typography>
       </Container>
       </Video>
       ...
       <Grid container bgcolor="whitesmoke" display="flex" flexWrap="wrap" justifyContent="center" gap={1} sx={{paddingY:"5px", marginTop:{xs:-4,md:-10}}}>
             {itemData.map((item)=>{
              ...
              }}><Stack  position="absolute" bottom={4} color="whitesmoke" direction="row" spacing={1}><LocationOn/><Typography variant="h6" fontWeight="bold">{t(item.title)}</Typography></Stack></Grid>
             })}
        </Grid>
    </Wrapper>
    )
